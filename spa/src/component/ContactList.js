    ...
    componentDidMount = async () => {
        ...
        axios.post('/api?type='+this.props.dbinfo.botable, {
            ...
            try{
                ...
                console.log(this.state.contactData) //리액트 변수에 데이터옮겨짐 데이터타입이 정렬이므로 map으로 풀어버림
            }
            catch(err){
    ...
    render() {
        return (
            <Accordion defaultActiveKey="0" flush className={ ' container  py-5'}  tag ='div' >
                <h2>{ this.state.contactData.length > 0 ? this.props.dbinfo.titlenm + "("+ this.state.contactData.length + ")" : this.state.message } </h2>
                    {
                                    ...
                                    <Accordion.Item eventKey={i.toString()} key={ content.keyno } >
                                        ...
                                        <Accordion.Header>
                                            ...
                                            <strong className='row justify-content-between align-items-center w-100'>
                                                <span className='col-sm-9 mb-0'> { content.company }</span>                                          
                                            </strong>
                                        </Accordion.Header>
                                        <Accordion.Body>
                                                { content.position }
